...
## Region names [NB copy of this also in pct01d_model_scripts/00_setup_and_funs - if modify here, modify there too]
...
download_link_national_data <- function(file, purpose, geography, formats = c('Rds', 'geojson', 'csv'), outputs_national_sha){
  #base_url = paste("https://cdn.rawgit.com/pctbike/pct01b_model_outputs_national", outputs_national_sha, purpose, geography, sep = "/")
  # ANNA NOTE - CAN'T GET CDN & SHA WORKING WITH LFS FILES...THIS JUST GOES TO LATEST MASTER VERSION
  base_url = paste("https://github.com/pctbike/pct01b_model_outputs_national/raw/master", purpose, geography, sep = "/")
  all_links <- ""
  for(i in 1:length(formats)){
    ...
    all_links <- paste(
      all_links, a(format,
                   ...
                   onclick= paste0("ga('send', 'event', 'download', '", file , '.' , format, "', '", outputs_national_sha ,"' );")
      )
    )
  }
  all_links
...
download_link_region_data <- function(file, purpose, geography, region, formats = c('Rds', 'geojson', 'csv'), outputs_regional_sha){
  #base_url = paste("https://cdn.rawgit.com/pctbike/pct01c_model_outputs_regional", outputs_regional_sha, purpose, geography, region, sep = "/")
  base_url = paste("https://github.com/pctbike/pct01c_model_outputs_regional/raw/master", purpose, geography, region, sep = "/")
  all_links <- ""
  for(i in 1:length(formats)){
    format <- formats[i]
    all_links <- paste(
      all_links, a(format,
                   ...
                   target='_blank',
                   download = paste0(file, ".", format),
                   onclick= paste0("ga('send', 'event', 'download', '", file , '.' , format, "', '", outputs_regional_sha ,"' );")
      )
    )
  }
  all_links
...
download_link_codebook <- function(file, purpose, repo_sha){
  base_url = paste("https://cdn.rawgit.com/pctbike/pct02_shiny_interface", repo_sha, "regions_www/www/static/02_codebooks", purpose, sep = "/")
  all_links <- ""
  all_links <- paste(
